...
def _build_source() -> MetricSource:
    ...
    # Punto de extensión: agregar "cloudwatch", "prometheus", etc.
    raise ValueError(f"DATA_SOURCE desconocido: {config.DATA_SOURCE!r}")


def collect_and_store() -> None:
    """Job de recolección. Los errores se loguean sin interrumpir el ciclo."""
    try:
        ...
        logger.info(
            ...
            config.INSTANCE_NAME, reading.cpu_pct, reading.ram_pct,
        )
    except Exception:
